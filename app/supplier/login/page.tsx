  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-slate-900 to-slate-800">
      <div className="bg-white rounded-2xl shadow-2xl p-8 w-full max-w-sm">
        ...
        <h1 className="text-2xl font-bold text-slate-800 text-center mb-1">Supplier portal</h1>
        <p className="text-sm text-slate-400 text-center mb-8">Sign in to your WG Trade supplier account</p>
        ...
        <form onSubmit={handleLogin}>
          ...
          <input type="password" value={password} onChange={(e) => setPassword(e.target.value)} required
            ...
            className="w-full px-3.5 py-2.5 border border-slate-200 rounded-lg text-sm mb-6 focus:outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 transition" />
        ...
        <p className="text-sm text-slate-400 text-center mt-5">
          New supplier?{' '}
          <Link href="/supplier/register" className="text-emerald-600 font-semibold hover:underline">Create account</Link>
        </p>
      </div>
    </div>
  );
